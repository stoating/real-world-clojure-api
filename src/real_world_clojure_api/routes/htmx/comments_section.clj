...
(defn comment-component
  ...
  ([comment opts]
   [:div.odd:bg-white.even:bg-slate-100.px-4.py-4.sm:px-6.lg:px-8
    opts
    [:p.text-gray-700.text-sm.linkify.break-normal
     (:comment comment)]
...
(defn comments-section-component
  ...
  [:div#comments-section.shadow-xl.mt-4
   ...
   #_[:h2.px-4.sm:px-6.lg:px-8.bg-gray-100.py-5.text-lg.font-medium.sm:text-xl
    (format "Comments (%s)" (count comments))]
   #_[:div#comments.mx-auto
    (for [comment comments]
      (comment-component comment))]
  ...
  {:name ::post
   ...
   (fn [context]
     (let [comment {:comment (-> context :request :params :comment)
           ...
           response
           (-> (list
                #_[:div {:hx-swap-oob "afterbegin:#comments"}
                 (comment-component comment)]
                (comment-form-component))
               (ok))]
